...
**30-second path:** run `npx -y @dinglebear/rapprise-mcp health --json` -> start loopback
HTTP with `APPRISE_MCP_HOST=127.0.0.1 npx -y @dinglebear/rapprise-mcp serve` -> call
...
| Binary / CLI | `rapprise` |
| npm package | `@dinglebear/rapprise-mcp` |
...
|---|---|---|---|
| npm / npx | `npx -y @dinglebear/rapprise-mcp --help` | Linux/Windows x86_64 clients. | Verifies the release archive SHA-256 before atomic install. |
...
```bash
npx -y @dinglebear/rapprise-mcp --help
npx -y @dinglebear/rapprise-mcp mcp
npx -y @dinglebear/rapprise-mcp health --json
```
...
```bash
APPRISE_MCP_HOST=127.0.0.1 npx -y @dinglebear/rapprise-mcp serve
```
...
|---|---:|---|---|
| MCP stdio | Supported | `rapprise mcp`, `npx -y @dinglebear/rapprise-mcp mcp` | Local child-process MCP clients. |
