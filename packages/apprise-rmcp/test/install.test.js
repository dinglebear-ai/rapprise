...
test("attestation verification pins repository, workflow, tag, and denies self-hosted signers", () => {
  let invocation;
  const result = verifyAttestation("archive.tar.gz", "archive.sigstore.json", "dinglebear-ai/rapprise", "v1.2.3", (command, args, options) => {
...
test("attestation verification fails closed", () => {
  assert.throws(
    () => verifyAttestation("archive.tar.gz", "archive.sigstore.json", "dinglebear-ai/rapprise", "v1.2.3", () => ({ status: 1 })),
